...
for image_number = 1:number_of_images;
  ...
  counts =  imhist(image_to_proccess);
  c = normalize(counts);
  c(c==0) = [];
  full_entropy = -sum(c .* log2(c));
  ...
  for x = 1 : subareas_count_x
    for y = 1 : subareas_count_y
      ...
      #entropy_var = entropy(tmp) / full_entropy;
    ...
    #scatter(quartile_deviations, entropies, 'DisplayName', num2str(image_number));
